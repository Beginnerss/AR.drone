...
public class ControlDroneActivity
    ...
    private boolean avoidbarrier = true;//记录是否有认为操作无人机，控制壁障程序与人为控制优先级
...
//            forceCombinedControlMode = false;
    ...
    public void onDeviceOrientationChanged(float[] orientation, float magneticHeading, int magnetoAccuracy) {
       /* System.out.println("-------------screenRotationIndex" + screenRotationIndex);
        System.out.println(Arrays.toString(orientation));
        System.out.println("第二參數" + magneticHeading);
        System.out.println("第三參數" + magnetoAccuracy);*/
    ...
    //Arduino蓝牙连接
    public void myArduinoControlFly() {
        ...
        if (pairedDevices.size() > 0) {
            for (BluetoothDevice device : pairedDevices) {
                ...
                try {
                    mBluetoothsocket = mDevice.createInsecureRfcommSocketToServiceRecord(UUID.fromString("00001101-0000-1000-8000-00805F9B34FB"));
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    //蔽障判断，判断是否起飞，是否在壁障范围
                    if (whethertakeoff = true && avoidbarrier == true) {
                        if ((char) buffer[0] == '0') {//向左飞
                            System.out.println("左飞");
                            droneControlService.setProgressiveCommandEnabled(true);
                            running = true;
                            flyOrientation(0.265f, -0.002f);
                        }
                        ...
                        if ((char) buffer[0] == '3') {//向后飞
                            System.out.println("后飞");
                            droneControlService.setProgressiveCommandEnabled(true);
                            running = true;
                            flyOrientation(0.015f, -0.265f);
                        }
                        ...
                        if ((char) buffer[0] == '2') {//向右飞
                            System.out.println("右飞");
                            droneControlService.setProgressiveCommandEnabled(true);
                            running = true;
                            flyOrientation(-0.265F, 0.005f);
                        }
